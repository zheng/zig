...
const Ref = struct {
    ...
    fn nlistPtr(self: Ref, macho_file: *MachO) *macho.nlist_64 {
        ...
        return switch (self.sym_type) {
            .global => &macho_file.globals.items[self.sym_index],
            .undef => &macho_file.undefs.items[self.sym_index],
        };
    }
};
...
fn allocateLocalSymbols(self: *MachO, match: MatchingSection, offset: i64) !void {
    ...
    while (true) {
        const atom_sym = if (atom.file) |file| blk: {
        ...
        for (atom.aliases.items) |index| {
            const alias_sym = if (atom.file) |file| blk: {
                const object = &self.objects.items[file];
                break :blk &object.symtab.items[index];
            } else &self.locals.items[index];
            alias_sym.n_value = @intCast(u64, @intCast(i64, alias_sym.n_value) + offset);
        }

        for (atom.contained.items) |sym_at_off| {
            const contained_sym = if (atom.file) |file| blk: {
                const object = &self.objects.items[file];
                break :blk &object.symtab.items[sym_at_off.local_sym_index];
            } else &self.locals.items[sym_at_off.local_sym_index];
...
pub fn createGotAtom(self: *MachO, key: GotIndirectionKey) !*Atom {
    ...
    const r_type: u4 = switch (self.base.options.target.cpu.arch) {
    ...
    switch (key.where) {
        .local => {
            try atom.relocs.append(self.base.allocator, .{
                .r_offset = 0,
                .r_addend = 0,
                .r_symbolnum = key.where_index,
                .r_zig_symtab_hint = .local,
                .r_pcrel = false,
                .r_length = 3,
                .r_extern = true,
                .r_type = r_type,
    ...
    switch (arch) {
        .x86_64 => {
            ...
            atom.relocs.appendAssumeCapacity(.{
                .r_offset = 3,
                .r_addend = 0,
                .r_symbolnum = dyld_private_sym_index,
                .r_zig_symtab_hint = .local,
                .r_pcrel = true,
                .r_length = 2,
                .r_extern = true,
                .r_type = @enumToInt(macho.reloc_type_x86_64.X86_64_RELOC_SIGNED),
            });
            // push %r11
            ...
            atom.relocs.appendAssumeCapacity(.{
                .r_offset = 11,
                .r_addend = 0,
                .r_symbolnum = self.dyld_stub_binder_index.?,
                .r_zig_symtab_hint = .undef,
                .r_pcrel = true,
                .r_length = 2,
                .r_extern = true,
                .r_type = @enumToInt(macho.reloc_type_x86_64.X86_64_RELOC_BRANCH),
            });
        },
        .aarch64 => {
            ...
            atom.relocs.appendAssumeCapacity(.{
                ...
                .r_length = 2,
                .r_extern = true,
                .r_type = @enumToInt(macho.reloc_type_arm64.ARM64_RELOC_PAGE21),
            });
            // add x17, x17, 0
            ...
            atom.relocs.appendAssumeCapacity(.{
                ...
                .r_length = 2,
                .r_extern = true,
                .r_type = @enumToInt(macho.reloc_type_arm64.ARM64_RELOC_PAGEOFF12),
            });
            // stp x16, x17, [sp, #-16]!
            ...
            atom.relocs.appendAssumeCapacity(.{
                ...
                .r_length = 2,
                .r_extern = true,
                .r_type = @enumToInt(macho.reloc_type_arm64.ARM64_RELOC_GOT_LOAD_PAGE21),
            });
            // ldr x16, [x16, 0]
            ...
            atom.relocs.appendAssumeCapacity(.{
                ...
                .r_length = 2,
                .r_extern = true,
                .r_type = @enumToInt(macho.reloc_type_arm64.ARM64_RELOC_GOT_LOAD_PAGEOFF12),
            });
            // br x16
    ...
    switch (arch) {
        .x86_64 => {
            ...
            atom.relocs.appendAssumeCapacity(.{
                .r_offset = 6,
                .r_addend = 0,
                .r_symbolnum = self.stub_helper_preamble_atom.?.local_sym_index,
                .r_zig_symtab_hint = .local,
                .r_pcrel = true,
                .r_length = 2,
                .r_extern = false,
...
fn resolveDsoHandle(self: *MachO) !void {
    if (self.strtab_dir.getKeyAdapted(@as([]const u8, "___dso_handle"), StringIndexAdapter{
        ...
        if (global.sym_type != .undef) break :blk;
...
fn resolveSymbolsInDylibs(self: *MachO) !void {
    ...
    loop: while (next_sym < self.unresolved.count()) {
        ...
        for (self.dylibs.items) |dylib, id| {
            ...
            var ref = self.refs.get(n_strx).?;
            while (true) {
                const object = &self.objects.items[ref.file.?];
                const sym = &object.symtab.items[ref.sym_index];
...
fn parseObjectsIntoAtoms(self: *MachO) !void {
    ...
    for (self.objects.items) |*object, object_id| {
        ...
        while (it.next()) |entry| {
            ...
            while (true) {
                ...
                log.warn("  {s}: n_value=0x{x}, size=0x{x}, alignment=0x{x}", .{
    ...
    while (it.next()) |entry| {
        ...
        const sect_size = if (self.atoms.get(match)) |last| blk: {
            const last_atom_sym = if (last.file) |file| inner: {
                const object = self.objects.items[file];
                break :inner object.symtab.items[last.local_sym_index];
            } else self.locals.items[last.local_sym_index];
...
pub fn deinit(self: *MachO) void {
    ...
    for (self.managed_refs.items) |ref| {
...
fn logSymtab(self: *MachO) !void {
    ...
    for (self.objects.items) |object| {
        ...
        for (object.symtab.items) |sym, i| {
            if (!symbolIsSect(sym) or symbolIsExt(sym)) continue;
    ...
    for (self.refs.keys()) |n_strx| {
        var ref = self.refs.get(n_strx).?;
        if (ref.sym_type != .global) continue;
        log.warn("  {s}: {}", .{ self.getString(n_strx), ref.nlist(self) });
        while (true) {
            const tt: []const u8 = if (ref.sym_type == .global) "GLOB" else "UND";
            if (ref.file) |file| {
                const object = self.objects.items[file];
                log.warn("    => ({s}) {d}: {s}", .{ tt, ref.sym_index, object.name });
            } else {
                log.warn("    => ({s}) {d}: null", .{ tt, ref.sym_index });
            }
            if (ref.next) |next| {
                ref = next;
            } else break;
    ...
    for (self.refs.keys()) |n_strx| {
        var ref = self.refs.get(n_strx).?;
        if (ref.sym_type != .undef) continue;
        log.warn("  {s}: {}", .{ self.getString(n_strx), ref.nlist(self) });
        while (true) {
            const tt: []const u8 = if (ref.sym_type == .global) "GLOB" else "UND";
            if (ref.file) |file| {

...
/// Extended macho.relocation_info struct.
pub const Relocation = struct {
    /// offset in the section to what is being relocated
    r_offset: u32,

    /// parsed addend (no need to reparse the instruction, or look for ARM64_RELOC_ADDEND)
    r_addend: i64,

    /// symbol index
    r_symbolnum: u32,

    /// if relocation targeting Zig module, hint which symtab to reference
    r_zig_symtab_hint: ?enum {
        local,
        undef,
    },
...
//     const sym_index = if (rel.r_extern == 0) outer: {
//         const sect_id = @intCast(u16, rel.r_symbolnum - 1);
//         const sym_index = context.object.sections_as_symbols.get(sect_id) orelse inner: {
...
//             const sym_index = @intCast(u32, context.object.symtab.items.len);
//             try context.object.symtab.append(context.allocator, .{
//                 .n_strx = 0,
//                 .n_type = macho.N_SECT,
...
//                 .n_value = 0,
//             });
//             try context.object.sections_as_symbols.putNoClobber(context.allocator, sect_id, sym_index);
//             break :inner sym_index;
//         };
//         break :outer sym_index;
//     } else rel.r_symbolnum;

//     parsed_rel.where_index = sym_index;
